...
namespace Z0
{
    ...
    void PrepareCaseWorkdirIfNeeded(int myid)
    {
        ...
        if (fs::exists(fs::path("Z0_NULL") / "CASE" / "setup" / "filenames"))
        {
            fs::current_path("Z0_NULL");
        }
        ...
        {
            fs::create_directories(fs::path("Z0_NULL") / "CASE");
            if (!fs::exists(fs::path("Z0_NULL") / "CASE" / "setup"))
            {
                try
                {
                    fs::create_directory_symlink(fs::path("..") / ".." / "Z4_Mercury" / "9999setup",
                                                 fs::path("Z0_NULL") / "CASE" / "setup");
                }
                catch (...)
                {
                    fs::create_directories(fs::path("Z0_NULL") / "CASE" / "setup");
                    for (const auto &entry : fs::directory_iterator(fs::path("Z4_Mercury") / "9999setup"))
                        fs::copy_file(entry.path(),
                                      fs::path("Z0_NULL") / "CASE" / "setup" / entry.path().filename(),
        ...
        if (myid == 0)
            std::cout << "Z0_NULL CASE working directory: " << fs::current_path().string() << "\n";
    }
}
