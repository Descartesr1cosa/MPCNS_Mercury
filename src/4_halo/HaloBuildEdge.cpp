...
void Halo::build_parallel_2DCorner_pattern(StaggerLocation loc, int nghost)
{
    ...
    for (const auto &ep : parallel_edges)
    {
        if (ep.is_coupling)
            continue;
        const Block &blk_this_for_count = fld_->grd->grids(ep.this_block);
        const Int3 this_blk_for_count = HALO_TOOLS::block_node_size(blk_this_for_count);
        if (boundary_dir_count(ep.this_box_node, this_blk_for_count) != 2)
            continue;
...
void Halo::build_inner_2DCorner_pattern(StaggerLocation loc, int nghost)
{
    ...
    for (const auto &ep : inner_edges)
    {
        if (ep.is_coupling)
            continue;
        const Block &blk_this_for_count = fld_->grd->grids(ep.this_block);
        const Block &blk_nb_for_count = fld_->grd->grids(ep.nb_block);
        const Int3 this_blk_for_count = HALO_TOOLS::block_node_size(blk_this_for_count);
        const Int3 nb_blk_for_count = HALO_TOOLS::block_node_size(blk_nb_for_count);
        if (boundary_dir_count(ep.this_box_node, this_blk_for_count) != 2 ||
            boundary_dir_count(ep.nb_box_node, nb_blk_for_count) != 2)
            continue;
