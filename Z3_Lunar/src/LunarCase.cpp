...
void PrepareCaseWorkdirIfNeeded(int myid)
{
    ...
    fs::path application;
    if (fs::exists(fs::path("Z3_Lunar") / "9999setup" / "filenames"))
        application = "Z3_Lunar";
    else if (fs::exists(fs::path("..") / "Z3_Lunar" / "9999setup" / "filenames"))
        application = fs::path("..") / "Z3_Lunar";
    else if (fs::exists(fs::path("9999setup") / "filenames"))
        application = ".";
    else
        throw std::runtime_error("LunarZ3: cannot locate Z3_Lunar/9999setup/filenames");
    ...
    if (myid == 0)
        std::cout << "LunarZ3 CASE working directory: " << fs::current_path() << '\n';
}
}
