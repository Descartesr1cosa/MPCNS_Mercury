...
void MercurySolver::PrintMinMaxDiagnostics_()
{
    ...
    for (int ib = 0; ib < fld_->num_blocks(); ++ib)
    {
        ...
        if (!UH.is_allocated() || !UNa.is_allocated() ||
            !PVH.is_allocated() || !PVNa.is_allocated() ||
    ...
    double mins_l[8] = {
        rhoH_min_l, pH_min_l, rhoNa_min_l, pNa_min_l,
        bx_min_l, by_min_l, bz_min_l, b2_min_l};
    double mins_g[8];
    double maxs_l[9] = {
        rhoH_max_l, pH_max_l, rhoNa_max_l, pNa_max_l,
    ...
    const double Babs_max = std::sqrt(std::max(0.0, 2.0 * maxs_g[7]));
    std::printf("\n           rhoH=[%.3e, %.3e]  pH=[%.3e, %.3e]\n",
