...
{
    const Box3 &inner = r.inner_slab;            // boundary-adjacent active slab
    const int ax = std::abs(r.direction);        // 1/2/3
    const int sgn = (r.direction > 0) ? +1 : -1; // +: high face, -: low face outward

    // outward step: active cell -> ghost cell
    const Int3 cyc = (ax == 1)   ? Int3{sgn, 0, 0}
                     : (ax == 2) ? Int3{0, sgn, 0}
                                 : Int3{0, 0, sgn};

    // face shift: high face uses +1, low face uses 0
    const int shift = (sgn > 0) ? 1 : 0;

    // ---- identify species from descriptor.name ----
    const std::string name = U.descriptor().name;

    // This branch is intended for Na+ conservative field, e.g. "U_Na".
    // Keep this test relatively strict to avoid catching unrelated fields accidentally.
    const bool is_Na =
        (name.find("U_Na") != std::string::npos) ||
        (name.find("Na") != std::string::npos);

    // ---- floors ----
    const double rho_floor = (is_Na) ? 1.0e-12 : 2.5e-3;
    const double p_floor = 1.0e-12;
    ...
    auto pressure_from_cons = [&](double rho,
                                  double mx,
                                  double my,
                                  double mz,
                                  double E) -> double
    {
        ...
        const double p = (bc_state_.gamma - 1.0) * eint;
        return std::max(p, p_floor);
    };

    // clamp indices into allocated range, including ghost cells
    ...
    // make normal always point toward the fluid side
    const double sign_to_fluid = (sgn > 0) ? -1.0 : +1.0;

    // loop over boundary-adjacent active cells
    for (int ii = inner.lo.i; ii < inner.hi.i; ++ii)
        for (int jj = inner.lo.j; jj < inner.hi.j; ++jj)
            for (int kk = inner.lo.k; kk < inner.hi.k; ++kk)
            {
                ...
                const double An2 = std::max(dot(A, A), 1.0e-30);
                const double An = std::sqrt(An2);

                // n_hat points from the wall/solid side into the fluid side
                std::array<double, 3> n_hat = scal(A, sign_to_fluid / An);

                // ============================================================
                // Na+ absorbing boundary:
                //
                // Do NOT overwrite the first active fluid cell.
                // Only fill ghost cells with low-density, low-pressure state.
                //
                // This represents precipitation/absorption at Mercury's surface:
                // Na+ can leave the computational fluid region through the wall,
                // but the wall does not inject Na+ back into the flow.
                // ============================================================
                if (is_Na)
                {
                    for (int ng = 1; ng <= ngh; ++ng)
                    {
                        ...
                        U(ig, jg, kg, 0) = rho_floor;
                        U(ig, jg, kg, 1) = 0.0;
                        U(ig, jg, kg, 2) = 0.0;
                        U(ig, jg, kg, 3) = 0.0;
                        U(ig, jg, kg, 4) = p_floor / (bc_state_.gamma - 1.0);
                    }
                ...
                // ============================================================
                // Original solid-wall treatment for non-Na species.
                // This keeps your previous H+ / ordinary-fluid behavior.
                // ============================================================

                // First layer: overwrite boundary-adjacent active cell
                // using one cell deeper in the fluid as reference.
                {
                    int ir = ii - cyc.i;
                    int jr = jj - cyc.j;
                    int kr = kk - cyc.k;
                    ...
                    // Current strong wall version: no-slip / stagnant wall.
                    // If later you want slip wall, replace this by:
                    // const double vn = dot(v, n_hat);
                    // std::array<double, 3> v_g = sub(v, scal(n_hat, vn));
                    std::array<double, 3> v_g = {0.0, 0.0, 0.0};
                ...
                // Fill ghost layers for non-Na species.
                for (int ng = 1; ng <= ngh; ++ng)
                {
                    int ir = ii;
                    int jr = jj;
