...
namespace TOPO
{
    ...
    EntityId Topology::id_of(const EntityKey &key) const
    {
        ...
        case EntityDim::Node:
        {
            ...
            const auto eq_it = nodes.local_to_rep.find(local);
        ...
        {
            const EntityKey local = key;
            const auto key_it = edges.local_to_qkey.find(local);
        ...
        {
            const EntityKey local = key;
            const auto key_it = faces.local_to_qkey.find(local);
    ...
    int Topology::sign_to_owner(const EntityKey &key) const
    {
        ...
        {
            const EntityKey local = key;
            const auto sign_it = edges.local_to_qsign.find(local);
        ...
        {
            const EntityKey local = key;
            const auto sign_it = faces.local_to_qsign.find(local);
    ...
    namespace detail
    {
            ...
            int dimension)
        {
            ...
            reconcile_node_equivalence_parallel_impl(equiv, my_rank, all_local_nodes, equiv, equiv.nodes.rep_count);
            ...
            build_edge_equivalence_from_nodes_impl(all_local_edges, equiv, edge_scratch);
            build_edge_entity_ids_impl(equiv);

            select_edge_owner_parallel_impl(equiv, edge_scratch, equiv.nodes.rep_count);
            ...
            build_face_equivalence_from_nodes_impl(all_local_faces, equiv, face_scratch);
            build_face_entity_ids_impl(equiv);

            select_face_owner_parallel_impl(equiv, face_scratch, equiv.nodes.rep_count);
