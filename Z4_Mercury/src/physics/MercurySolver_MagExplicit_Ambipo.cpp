...
void MercurySolver::AddAmbipolarEdgeEMF_()
{
    ...
    auto pe_cell = [](FieldBlock &PVH, FieldBlock &PVN,
                      int i, int j, int k) -> double
    {
        return PVH(i, j, k, 3) + PVN(i, j, k, 3);
    };

    auto pe_node = [&](FieldBlock &PVH, FieldBlock &PVN,
                       int i, int j, int k) -> double
    {
        double sum = 0.0;
    ...
    auto edge_num_xi = [&](FieldBlock &UH, FieldBlock &UNa,
                           int i, int j, int k) -> NumInfo
    {
        double rhoH = 0.0;
        double rhoNa = 0.0;
        ...
        for (int dj = -1; dj <= 0; ++dj)
            for (int dk = -1; dk <= 0; ++dk)
            {
                ...
                rhoNa += UNa(i, j + dj, k + dk, 0);
            }

        return Hall_Num_Limiter(0.25 * rhoH, 0.25 * rhoNa);
    };

    auto edge_num_eta = [&](FieldBlock &UH, FieldBlock &UNa,
                            int i, int j, int k) -> NumInfo
    {
        double rhoH = 0.0;
        double rhoNa = 0.0;
        ...
        for (int di = -1; di <= 0; ++di)
            for (int dk = -1; dk <= 0; ++dk)
            {
                ...
                rhoNa += UNa(i + di, j, k + dk, 0);
            }

        return Hall_Num_Limiter(0.25 * rhoH, 0.25 * rhoNa);
    };

    auto edge_num_zeta = [&](FieldBlock &UH, FieldBlock &UNa,
                             int i, int j, int k) -> NumInfo
    {
        double rhoH = 0.0;
        double rhoNa = 0.0;
    ...
    for (int ib = 0; ib < nb; ++ib)
    {
        ...
        auto &PVN = fld_->field(fid_.fid_PV_Na, ib);
...

        {
            ...
            for (int i = lo.i; i < hi.i; ++i)
                for (int j = lo.j; j < hi.j; ++j)
                    ...
                    {
                        const double pe0 = pe_node(PVH, PVN, i, j, k);
                        const double pe1 = pe_node(PVH, PVN, i + 1, j, k);
                        const NumInfo num = edge_num_xi(UH, UNa, i, j, k);
...

        {
            ...
            for (int i = lo.i; i < hi.i; ++i)
                for (int j = lo.j; j < hi.j; ++j)
                    ...
                    {
                        const double pe0 = pe_node(PVH, PVN, i, j, k);
                        const double pe1 = pe_node(PVH, PVN, i, j + 1, k);
                        const NumInfo num = edge_num_eta(UH, UNa, i, j, k);
...

        {
            ...
            for (int i = lo.i; i < hi.i; ++i)
                for (int j = lo.j; j < hi.j; ++j)
                    ...
                    {
                        const double pe0 = pe_node(PVH, PVN, i, j, k);
                        const double pe1 = pe_node(PVH, PVN, i, j, k + 1);
                        const NumInfo num = edge_num_zeta(UH, UNa, i, j, k);
